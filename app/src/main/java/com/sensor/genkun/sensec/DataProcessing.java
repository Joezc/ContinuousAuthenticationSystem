...
public class DataProcessing {
    ...
    public void readData(int k, String fileName, double seconds) {
        ...
        switch (k) {
            case 0: accelerometer = new double[4][total];break;
            case 1: orientation = new double[4][total];break;
            case 2: magnetic = new double[4][total];break;
            case 3: gyroscope = new double[4][total];
        }
        ...
        for (int i = 0; i < lines.length && i < total; ++i) {
            ...
            for (int j = 0; j < values.length; ++j) {
                if (!Objects.equals(values[j], "")){
                    switch (k) {
                        case 0: accelerometer[j][i] = Double.parseDouble(values[j]);break;
                        case 1: orientation[j][i] = Double.parseDouble(values[j]);break;
                        case 2: magnetic[j][i] = Double.parseDouble(values[j]);break;
                        case 3: gyroscope[j][i] = Double.parseDouble(values[j]);
                    }
                }
            }
        }
