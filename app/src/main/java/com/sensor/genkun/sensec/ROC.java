...
public class ROC {
    ...
    public static double preProcessing(int sampleId, int windowId) {
        ...
        String[] stringNegative = new String[MainActivity.MODEL_NUM - 1];
        try {
            stringPositive = TextFile.readFile(MainActivity.rootPath + sampleId + " - " + sampleId + ".txt");
            int k = 0;
            for (int i = 1; i <= MainActivity.MODEL_NUM; ++i) {
                if (i != sampleId) {
                    stringNegative[k] = TextFile.readFile(MainActivity.rootPath + sampleId + " - " + i + ".txt");
                    ++k;
                }
            }
        } catch (Exception e) {
        ...
        switch (windowId) {
            case 0: roc(positive, negative, 3);break;
            case 1: roc(positive, negative, 9);break;
            case 2: roc(positive, negative, 20);break;
            case 3: roc(positive, negative, 40);break;
            case 4: roc(positive, negative, 80);break;
        }
    ...
    public static void roc(double[] positive, double[] negative, double initPt) {
        ...
        for (;pt <= 0; pt += step) {
            ...
            try {
                TextFile.writeFile(MainActivity.rootPath + initPt + "FRR" + ".txt", "" + FRR(positive, pt) + "\t", true);
                TextFile.writeFile(MainActivity.rootPath + initPt + "FAR" + ".txt", "" + FAR(negative, pt) + "\t", true);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
